...
void setAngle(int angle){
    ...
    ModuleState::MODULE_EXITCODE Proxy::body() 
    {
        ...
        while (getModuleState() == ModuleState::RUNNING) 
        {
          ...
          /*  Container containerSteeringDataDriver = getKeyValueDataStore().get(Container::USER_DATA_2);
	    SteeringData sdDriver = containerSteeringDataDriver.getData<SteeringData> ();  */

	    Container containerSteeringDataLane = getKeyValueDataStore().get(Container::USER_DATA_1);
	    SteeringData sd = containerSteeringDataLane.getData<SteeringData> ();

	    
			//if(sdDriver.getExampleData() > 0 || sdDriver.getExampleData() < 0)
			//{
...
if (sbd.SensorBoardData::getValueForKey_MapOfDistances(0) < 0 && sbd.SensorBoardData::getValueForKey_MapOfDistances(2) <=0 && overtaking == 2){
    ...
    *povertaking = 0; }
			...
			{
				angle = sdLane.getExampleData();
				printf("Lane detection angle is:  %d\n", angle);
				setAngle(angle);
			} */
